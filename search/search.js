...
function snippet(result) {
  ...
  for (term in result.matchData.metadata) {
    if ("caption" in result.matchData.metadata[term]) {
      p = p.concat(result.matchData.metadata[term].caption.position);
    }
  }
  ...
  const caption = documents[result["ref"]]["caption"];
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    if(numMatches >= bestMatches) {
      ...
      COM /= numMatches;
      const idealStart = Math.min(COM - SNIPPET_MAX_CHARS / 2, caption.length - SNIPPET_MAX_CHARS);
